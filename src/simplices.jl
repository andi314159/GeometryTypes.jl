...
@inline function (::Type{Simplex{S, T}})(x) where {S, T}
    Simplex{S, T}(ntuple(i-> T(x), Val{S}))
end
@inline function (::Type{Simplex{S}})(x::T) where {S, T}
    Simplex{S, T}(ntuple(i-> x, Val{S}))
end
@inline function (::Type{Simplex{1, T}})(x::T) where T
    Simplex{1, T}((x,))
end
@inline (::Type{Simplex})(x::NTuple{S}) where {S} = Simplex{S}(x)
@inline function (::Type{Simplex{S}})(x::T) where {S, T <: Tuple}
